#!/bin/bash
# Docker Compose interactive status viewer for herdr popup
# Uses fzf for fuzzy filtering, log preview, and container actions.
# Standalone copy of tmux/scripts/docker-compose-status.sh — no cross-tool
# dependency; cwd is resolved from HERDR_ACTIVE_PANE_CWD instead of tmux's
...
compose_dir=$(find_compose_dir "${HERDR_ACTIVE_PANE_CWD:-$PWD}")
